...
void sha256_final(uint32_t *state, uint8_t hash[], uint8_t *buffer, size_t buffer_len, uint64_t bit_len) {
    ...
    // Append the total length of the message in bits
    buffer[63] = bit_len;
    buffer[62] = bit_len >> 8;
    ...
    // Convert the hash state to big-endian output
    for (i = 0; i < 8; ++i) {
...
uint8_t* sha256(const uint8_t *data, size_t len) {
    ...
    size_t buffer_len = 0;
    uint64_t bit_len = 0;
    static uint8_t hash[32];  // Static to ensure the hash persists after the function returns
